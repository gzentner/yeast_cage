...
```{r}
# Cluster TSSs
exp <- tss_clustering(exp, max_distance = 25, max_width = 250, threshold = 20)
...
conditions <- list(quantile_by = "shape_index", n_quantiles = 5, filters = "iqr_width > 1")

seqs <- tss_sequences(exp, dominant = TRUE, data_conditions = conditions, 
                      samples = "ScerYPD.1", threshold = 20)
...
```{r}
# Color map (Fig. 2D)
#plot_sequence_colormap(seqs, samples = "ScerYPD.1")
```

```{r fig.align = "center", fig.cap = "Dinucleotide frequencies for YPD replicate 1"}
# Dinucleotide frequencies (Fig. 2E)
frequencies <- dinucleotide_frequencies(exp, dominant = FALSE, samples = "ScerYPD.1")
...
```{r fig.align = "center", fig.height = 10, fig.cap = "Dot plot of GO biological process enrichment in upregulated and downregulated TSRs in for YPG vs. YPD comparison"}
# Annotate DE TSRs
